...
def format_from_rich(rich_format):
    return b"<" + b"".join(x[0].encode() for x in rich_format)
...
class ArticleEntryFormat(Format):
    ...
    def unpack_from_file(self, f, seek=None):
        d = super().unpack_from_file(f, seek)
...
class MimeTypeListFormat(Format):
    def __init__(self):
        super(MimeTypeListFormat, self).__init__(b"")
...
class ZimFile(object):
    ...
    def get_uuid(self):
        ...
        uuid_bytes = [h['uuid0'], h['uuid1'], h['uuid2'], h['uuid3'], h['uuid4'],
                      ...
                      h['uuid15']]
        s = b"".join(bytearray((x,)) for x in uuid_bytes)
    ...
    def list_articles_by_url(self):
        ...
        for i in range(self.header['articleCount']):
            entry = self.read_directory_entry_by_index(i)
            # print(full_url(entry['namespace'], entry['url']))
            s += full_url(entry['namespace'], entry['url']) + b"\n"
        return s

import pdb
from bs4 import BeautifulSoup

# def cleanBytesHTML(bytesHtml):
dataDirectoryName = "/Users/sammy/Documents/phd-2016/ai-lab/data/"
def main():
    with open('toto.html', 'wb') as fil:
        zimfile = ZimFile(dataDirectoryName + '/wikinews_fr_all_2016-12.zim')
        numberArticles = 1000
        i = 0
        n  = len([za for za in zimfile.articles()])
        textList = []
        for entry in itertools.islice(zimfile.articles(), numberArticles):
            content, mime_type, *_ = zimfile.get_article_by_url(entry['namespace'], entry['url'])
            if mime_type != b'text/html':
                continue
            soup = BeautifulSoup(content, "lxml")
            # wr.writeParserResults(self.data[1:], directoryName + "/localFiles/" + nameFile + ".shelve", shelve)
            article_text = soup.get_text()
            text_tokens = nltk.word_tokenize(article_text)
            textList = textList + text_tokens
            if i % 500 == 0:
                print(numberArticles-i)
            i = i +1

        # textList = list(set(textList))
        tokenizedStr = textList[0]
        for tl in textList[1:]:
            tokenizedStr = tokenizedStr + " " + tl

        # wr.writeParserResults(tokenizedStr, dataDirectoryName + "wikinews_fr_all_2016-12.shelve", shelve)
        with open(dataDirectoryName + "/wikinews_fr_all_2016-12.txt", "w") as f:
            f.write(tokenizedStr)
        with open(dataDirectoryName + "/wikinews_fr_all_2016-12.txt", "r") as f:
            readTokenizeStr = f.read()
        pdb.set_trace()
